...
Deep active-subspace pipeline. Input–output pairs (ξi, yi) are drawn directly from the simulator (no gradient samples needed); a nonlinear encoder hθ compresses the high-dimensional input into a d-dimensional latent code, a small link network gθ maps the latent code to the response, and an elastic-net / validation-MSE elbow chooses d. The trained composition f̂θ = gθ ∘ hθ is the deployed surrogate. Compared with the linear active-subspace pipeline (Figure {ref}`fig-active_subspace_pipeline`), the encoder + link boxes replace the eigendecomposition + linear projection, the elbow box replaces the spectral-gap search, and the gradient-sampling step disappears entirely.
```
