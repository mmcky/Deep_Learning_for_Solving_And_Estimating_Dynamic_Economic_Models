...
**[Core\] Operator learning vs. PINN.** For an option pricing problem where the strike $K$ varies across $\{45,46,\ldots,55\}$, count the training cost of (a) eleven independent PINN runs vs. (b) one operator-learning or parametric-PINN run that takes $K$ as an input; see {ref}`sec-operator_learning_bridge` and Chapter {ref}`ch-outlook`. At what cost ratio does amortized training win?
```
