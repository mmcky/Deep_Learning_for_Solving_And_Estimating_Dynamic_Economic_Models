...
- : a library-free Random Search loop (model in TF/Keras) on a 10-dimensional analytical regression task, used to illustrate the projection argument of {cite:t}`bergstra2012random` in its cleanest form.

- : from-scratch Random Search and Successive Halving (Hyperband's inner loop) on a two-dimensional Genz Gaussian, written in $\sim$`<!-- -->`{=html}25 lines of plain Python so the algorithms in this chapter are visible without library abstraction; after the first run, the cached records in short-circuit re-runs for instant re-inspection.

- : the classroom ReLoBRaLo implementation, matched to the notation below.

- : the IRBC exercise notebook (closed-form steady-state comparative statics and inverse-loss weighting on a multi-component IRBC residual); it is the notebook referenced by Chapter {ref}`ch-irbc` {prf:ref}`ex-ch3-6`--{prf:ref}`ex-ch3-7`, and it reuses the loss-balancing ideas of this chapter on a deliberately small, library-free example.
