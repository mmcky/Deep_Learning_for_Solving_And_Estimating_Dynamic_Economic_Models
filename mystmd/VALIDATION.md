...
**Errata (2026-08-16).** The figure count in R28's regression gates originally read **93**. That was wrong, and it contradicted the count of **88** that every round since R6 has reported. The AST-metrics script written for this round bucketed *any* container that was not `kind: table` as a figure, so it silently folded in the 5 `{code-block}` containers (88 + 5 = 93). Corrected above to 88 figures and 5 code blocks, counted by `kind`. The comparative claims are unaffected — before and after were measured with the same script, so "byte-identical" held regardless — but the absolute number was not the book's figure count and should not be cited as one. The same flaw later surfaced as a phantom 94th figure once `index.md` gained the epigraph, which is a `quote` container.
...
> The AST-metrics script described below over-counted figures by folding in `{code-block}` containers. See the **Errata** note in §1 for the correction — 88 figures, not 93 — and why the comparative claims are unaffected.
