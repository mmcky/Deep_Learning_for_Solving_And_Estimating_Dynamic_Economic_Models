...
## 1. Headline result (Round 22 — visual-parity audit)
...
3. **[QE#186](https://github.com/QuantEcon/claude-latex-to-myst/issues/186)** — unlabelled `equation`/`align` envs lose their numbers (LaTeX numbers them; converter emits plain `$$`), so downstream equation numbers drift from the PDF: ch01 −3 by (1.12), ch07 −3 across the HJB run; ch02/ch09/ch11 sampled points match (early eqs all-labelled) — invisible to all 21 structural rounds. **No practical local stopgap**; documented deviation until the converter preserves source-env numbering semantics.
