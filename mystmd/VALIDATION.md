...
**Round 28 update:** 2026-08-16 — **the strict coupled pin bump** (closes book[#19](https://github.com/mmcky/Deep_Learning_for_Solving_And_Estimating_Dynamic_Economic_Models/issues/19)): tool `b01fa92` → `182214f` (3 commits) **and** CI renderer `qe-v9` → `qe-v10` in the same commit. R27 deferred exactly this because [QE#160](https://github.com/QuantEcon/claude-latex-to-myst/issues/160) was open; [QE#209](https://github.com/QuantEcon/claude-latex-to-myst/pull/209) (#160A) has since landed, so the blocked half is now executable. Source pin unchanged (`8c37a8b`). **The starred sections finally lose their spurious numbers.** `\section*` is unnumbered in LaTeX; the converter now re-emits pandoc's `.unnumbered` class as a bare `{.unnumbered}` attribute block, which the renderer reads as `enumerated: false` — no number, no counter advance, target and TOC entry kept. **Output diff is 24 changed lines and nothing else**: 12 `## Exercises`, 11 `## Further Reading`, 1 `### Validation Protocol`, each gaining ` {.unnumbered}`. That is exactly the source population — the `.tex` contains precisely 12 `\section*{Exercises}`, 11 `\section*{Further Reading}` and 1 `\subsection*{Validation Protocol}` and no other starred sectioning command, so coverage is complete and nothing was over-reached. The 4 `\chapter*` frontmatter pages (Preface, Notation, How to Read, Execution Map) stay absorbed into their page `title:` as designed, confirmed untouched. **This coupling is the unforgiving kind**, unlike R26's: on qe-v9 the block would render as literal braces in the heading title *and* pollute its auto-slug, so a stale renderer corrupts pages rather than silently forfeiting a fix. Verified against a real qe-v10 build rather than assumed: **0 headings with a literal brace in the title, 0 slugs containing `unnumbered`**; the rendered `<h2>` emits `<span class="heading-text">Exercises</span>` with no number span, where its numbered sibling still carries `<span class="mr-3">3.6</span>`. **No counter drift** — the failure mode #160A fixed in dp2 (an unnumbered heading advancing the counter and renumbering its siblings) does not occur here: all **213 other headings are byte-identical**, enumerator for enumerator. ch03 is the only mid-chapter case (`Validation Protocol` was 3.6.1) and it is the sole subsection of §3.6, so nothing followed it to renumber. **Build stays 2 warnings / 0 errors**, the by-design kept-`\paragraph` pair ([#165](https://github.com/QuantEcon/claude-latex-to-myst/pull/165): `sec-irbc_fischer_burmeister` in ch03, `sec-matern` in ch09). **Regression gates all hold, measured as before/after AST diffs against a qe-v10 baseline build of the pre-bump markdown** (which isolates converter effects from renderer effects — the renderer bump alone produced zero new warnings): headings 237 → 237 with enumerated 234 → **210** and 24 now `enumerated: false`; **813 cross-references, 0 unresolved, byte-identical**; equation enumerators **byte-identical** per chapter and entry for entry (253 numbered nodes), preserving R26's 272/272 PDF match; figures (93), tables (46), proofs (74) and lists (140: 108 unordered, 32 ordered) all byte-identical; deep links preserved (`id="exercises"` ×12, `id="further-reading"` ×11, `id="validation-protocol"` ×1 in the built HTML, all with TOC anchors); Class L intact (10 escaped markers — appF ×6, ch02 ×4); 0 escaped roles, 0 marker leaks, 0 KaTeX errors, 0 TikZ churn. **Local wrapper synced** with the upstream template's new renderer-floor note; the tool's own `--build` stage now parses `myst --version` and warns below qe-v10. **Newly noted upstream, no local action:** [QE#210](https://github.com/QuantEcon/claude-latex-to-myst/issues/210) — qe-v10 consumes a trailing brace group in a heading *title* when it parses as attributes; latent here (0 occurrences book-wide).
...
## 1. Headline result (Round 28)

Tool pin **`b01fa92` → `182214f`** (3 commits) **and** renderer pin **`qe-v9` → `qe-v10`**, moved together in one commit — the strict coupling recorded in book[#19](https://github.com/mmcky/Deep_Learning_for_Solving_And_Estimating_Dynamic_Economic_Models/issues/19), which this round **closes**. Source pin unchanged (`8c37a8b`). [QE#209](https://github.com/QuantEcon/claude-latex-to-myst/pull/209) (issue [#160](https://github.com/QuantEcon/claude-latex-to-myst/issues/160)) emits `{.unnumbered}` on starred sections, and qe-v10 ([QuantEcon/mystmd#89](https://github.com/QuantEcon/mystmd/pull/89)) is the renderer that can parse it. The 24 spurious section numbers R27 documented as "still outstanding, by design" are now gone.

| | R27 | R28 |
|---|---|---|
| build warnings | 2 | 2 |
| — kept-`\paragraph` heading depth (by design, [#165](https://github.com/QuantEcon/claude-latex-to-myst/pull/165)) | 2 | 2 |
| headings carrying an enumerator | 234 | **210** |
| headings correctly unnumbered | 0 | **24** |
| starred sections in source not yet honoured | 24 | **0** |
| equation numbers vs printed PDF | 272/272 | 272/272 |
| cross-references / unresolved | 813 / 0 | 813 / 0 |

**Output diff is 24 changed lines and 0 anything else**: 12 `## Exercises`, 11 `## Further Reading`, 1 `### Validation Protocol`, each gaining ` {.unnumbered}`. That matches the source population exactly — the `.tex` holds precisely 12 `\section*{Exercises}`, 11 `\section*{Further Reading}`, 1 `\subsection*{Validation Protocol}`, and no other starred sectioning command. Complete coverage, no over-reach. The 4 `\chapter*` frontmatter pages remain absorbed into their page `title:`, as R27 said they should.

**This coupling is the unforgiving kind.** R26's `qe-v9` coupling failed *silently* on a stale renderer (it forfeited per-row equation numbering). This one **corrupts pages**: without a heading-attribute parser, `{.unnumbered}` renders as literal text in the title and pollutes the derived slug. So the gate was checked against a real qe-v10 build, not inferred — **0 headings with a literal brace in the title, 0 slugs containing `unnumbered`**. The rendered markup is the proof: an unnumbered heading emits `<h2 id="exercises"><span class="heading-text">Exercises</span>`, with no number span, while its numbered sibling still carries `<span class="mr-3 select-none">3.6</span>`.

**No counter drift.** The bug #160A fixed in dp2 — an unnumbered heading still advancing the counter and renumbering its numbered siblings — does not occur here. All **213 other headings are byte-identical**, enumerator for enumerator. Eleven of the twelve chapters put their starred sections last, so nothing follows them; ch03 is the one mid-chapter case (`Validation Protocol` held 3.6.1) and it is the sole subsection of §3.6, so again nothing follows.

**Method note — the baseline separates the two pins.** Because both pins moved at once, "no regressions" was measured against a qe-v10 build of the *pre-bump* markdown, not against the deployed qe-v9 site. That isolates renderer effects from converter effects. The renderer bump alone contributed **zero** new warnings and zero AST deltas, so every change below is attributable to the converter.

**Regression gates.** Headings 237 → 237 (none lost). Cross-references **813, 0 unresolved, byte-identical**. Equation enumerators **byte-identical** per chapter and entry for entry (253 numbered nodes under this count), so R26's 272/272 PDF match carries forward untouched. Figures (93), tables (46), proofs (74) and lists (140: 108 unordered, 32 ordered) byte-identical. Deep links preserved: `id="exercises"` ×12, `id="further-reading"` ×11, `id="validation-protocol"` ×1 in the built HTML, each with its TOC anchor — the fix keeps the target and the TOC entry, dropping only the number. Class L intact (10 escaped markers: appF ×6, ch02 ×4). 0 escaped roles, 0 marker leaks, 0 KaTeX errors, 0 TikZ churn.

**Also this round:** the local `convert.sh` wrapper picked up the upstream template's renderer-floor note, and the tool's `--build` stage now parses `myst --version` and warns when the renderer is below qe-v10 — previously it only complained when `myst` was missing entirely, the one case where nothing is built and no damage is possible.
...
**Method.** Extract the text of the 331-page PDF with `pdftotext`. For every sentence, take the longest run of consecutive *plain words* (ASCII letters, 3+ characters — single letters and greek are maths variables, digits carry reference numbers), keep runs of 8 words or more, and test whether that run appears in the **rendered HTML**. Both sides are normalised to `[a-z0-9]` only, so hyphenation, line-break joins, smart quotes, spacing and markup cannot produce a false miss. A run must be contiguous in the source text, so any intervening maths breaks it — that is what keeps the test from being merely permissive.

**Comparing against rendered HTML rather than the markdown is essential**, and getting this wrong is why a first pass badly understated fidelity. Four constructs exist in the markdown only as roles and materialise at render time: the deliberate doubled-noun strips from R18/R22 (`see {numref}`fig-x`` renders as "see Figure 2.3"), `{cite:t}` citations, `{ref}` cross-references, and maths. Comparing to the markdown flags all four as losses when every one of them renders correctly. A second trap: KaTeX emits **both** a MathML tree and a visual HTML tree per formula, so naive tag-stripping doubles every symbol (`k` → `k k`) and breaks any run crossing inline maths — the MathML subtree must be dropped.
...
|---|---|---|
| 124 | citation or maths rendering | test artifact — *none of these match their own LaTeX source either*, which is what proves the failure is the comparison method, not the conversion |
...
| 4 | run-boundary artifacts | the run ended inside maths; the interior fragment does render |
...
**What this does and does not establish.** It covers 39% of PDF sentences — those with a long enough plain-prose run. Sentences that are mostly maths, or short, are not reachable by text comparison and rest on the structural gates instead. Within its scope the result is a census, not a sample: every eligible sentence was tested, and exactly one real loss survived triage.
...
- **Paragraph-level content faithfulness** — *superseded for prose by §1.1 (Round 28)*, which ran a whole-book census against the printed PDF: 2,223 sentences (39% of the book) compared verbatim against the rendered site, with 1 confirmed content loss. What remains uncovered is the other 61% — sentences too short or too maths-dense for text comparison, which rest on the structural gates instead. The original note stands for those: the two spot-checks (§4.1, §4.2) cover ~3 pages of 329.
