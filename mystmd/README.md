...
> **These two pins move together.** `.tool-version` (which converter generates the markdown) and the `git checkout qe-v10` in [`../.github/workflows/deploy-myst.yml`](../.github/workflows/deploy-myst.yml) (which renderer builds the site) are coupled. Check the renderer floor whenever you move `.tool-version`. There are now two couplings in force, and they fail differently:
>
...
>
> So: diff the built equation numbers against the PDF rather than against the previous build, and check that no heading title contains a literal `{`. `myst --version` reports `v1.10.1 (qe-v10)` — only the parenthesised tag distinguishes the fork from upstream npm `mystmd`, and `convert.sh --build` now warns when it is too old.
