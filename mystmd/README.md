...
|---|---|---|
| [#19](https://github.com/mmcky/Deep_Learning_for_Solving_And_Estimating_Dynamic_Economic_Models/issues/19) | 24 starred headings (`\section*{Further Reading}` / `\section*{Exercises}`, plus ch03's `\subsection*{Validation Protocol}`) still carry section numbers they should not. Needs a converter bump **and** `qe-v10` in one commit — on `qe-v9` an `{.unnumbered}` attribute block leaks as literal braces into the heading text and its slug. | [QE#160](https://github.com/QuantEcon/claude-latex-to-myst/issues/160) emitting `{.unnumbered}` on starred sections |
