...
This continuous-time PINN approach and the discrete-time DEQN with Young's method (Chapter {ref}`ch-young`) address the *same* economic question – how heterogeneous agents interact through prices when markets are incomplete – but differ in the mathematical formulation summarized in {numref}`tab-discrete_continuous_ha`.
