...
(tab-bm_running_case)=
  ****Method****          **What is approximated**                            **Loss / objective**                                  **Diagnostic**                                       **Notebook**
  ----------------------- --------------------------------------------------- ----------------------------------------------------- ---------------------------------------------------- --------------
  **DEQN**                Savings share $s(K,z)\in(0,1)$ via sigmoid head     Squared rel. Euler residual on simulated trajectory   $s \to \alpha\beta$ at $\delta\!=\!1$, log utility   
  **PINN, cake-eating**   Value function $\hat V(a)$ on a 1D interval         HJB residual $\rho V-\max_c[u(c)+V'(a)(ra-c)]$        $\hat V$ matches closed form $V^\star$ (CRRA)        
  **GP $+$ BAL**          $\hat V(K)$ on Brock--Mirman ergodic set            GP marginal likelihood; BAL picks next $K_*$          \% pred. band covers true $V$                        
...
5.   **[Advanced/project\] Hybrid pipeline: DEQN $+$ GP $+$ SMM end-to-end.** Build a complete estimation pipeline on the Brock--Mirman model with two parameters $(\beta, \varrho)$, matching the parameter pair worked in the companion notebook . As that notebook documents, $\beta$ is only *partially* identified by macro moments (a visible ridge along the $\beta$ axis); $\varrho$ has the cleaner identification map. The exercise below is a controlled-difficulty version of that pipeline.
