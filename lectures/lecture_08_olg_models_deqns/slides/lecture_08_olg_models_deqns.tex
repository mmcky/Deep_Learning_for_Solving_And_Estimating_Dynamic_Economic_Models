...
\vspace{-0.4cm}
\begin{center}
{\footnotesize Companion notebooks \texttt{lecture\_08\_07}\,--\,\texttt{lecture\_08\_11} (in \texttt{lecture\_08\_olg\_models\_deqns/code/}): \texttt{\_07}/\texttt{\_08} the analytic 6-agent model, \texttt{\_09}/\texttt{\_10} the 56-cohort benchmark, \texttt{\_11} the student exercise.  Full filenames appear on the Roadmap slide and in Parts II--III.}\\[3pt]
...
\vspace{-3pt}\begin{itemize}\itemsep0pt
    ...
    \item Primary notebook: \texttt{lecture\_08\_08\_OLG\_Analytic\_DEQN\_persistent.ipynb}; companion ablation \texttt{lecture\_08\_07\_OLG\_Analytic\_DEQN\_exogenous.ipynb}.
\end{itemize}
\end{block}
\vspace{-2pt}
...
\vspace{-3pt}\begin{itemize}\itemsep0pt
    ...
    \item Primary notebook: \texttt{lecture\_08\_10\_OLG\_Benchmark\_DEQN\_persistent.ipynb}; companion ablation \texttt{lecture\_08\_09\_OLG\_Benchmark\_DEQN\_exogenous.ipynb}.
\end{itemize}
\end{block}
\end{frame}
...
\scriptsize
\emphc{Open notebook \texttt{lecture\_08\_08\_OLG\_Analytic\_DEQN\_persistent.ipynb}} (the persistent-simulation primary; the exogenous-sampling companion is \texttt{lecture\_08\_07\_OLG\_Analytic\_DEQN\_exogenous.ipynb}).  It implements the model, equilibrium conditions, loss, FREE map, architecture, training, and validation covered in Part II.
\vspace{-2pt}
...
\vspace{2pt}
The actual training run, DEQN-vs-analytic comparison, and Euler-error histogram are produced by \texttt{lecture\_08\_08\_OLG\_Analytic\_DEQN\_persistent.ipynb} (\S 8).  Production target in AGS (2022): mean relative Euler error $\lesssim 10^{-3}$.
\end{block}
\end{frame}
...
Open \texttt{lecture\_08\_08\_OLG\_Analytic\_DEQN\_persistent.ipynb}.  Set \texttt{RUN\_MODE = "teaching"} ($\sim 5$ min) for the classroom run; \texttt{"smoke"} (the default) is a $\sim$30-s sanity check; \texttt{"production"} chases AGS Table~3 accuracy.  Both notebooks expose a \texttt{SAMPLING\_MODE = "simulation" / "exogenous"} switch (the persistent primary uses \texttt{"simulation"}, the companion \texttt{lecture\_08\_07\_OLG\_Analytic\_DEQN\_exogenous.ipynb} uses \texttt{"exogenous"}); the model and loss are identical -- only the training cloud differs.
\end{block}
\end{frame}
...
\begin{block}{Sizes from \texttt{lecture\_08\_10\_OLG\_Benchmark\_DEQN\_persistent.ipynb}}
\centering
\begin{tabular}{ll}
\toprule
...
\vspace{-3pt}
For each aggregate state $z \in \{1,2,3,4\}$ the trained policy yields a lifecycle consumption profile $c^s(z)$ over ages $s=25,\ldots,80$, read off a long ergodic simulation from \texttt{lecture\_08\_10\_OLG\_Benchmark\_DEQN\_persistent.ipynb}.  (The notebook ships pooled lifecycle bands and residual diagnostics; the per-shock breakdown is a small add.)
\end{block}
...
\vspace{-2pt}
\textbf{Hands-on: \texttt{lecture\_08\_10\_OLG\_Benchmark\_DEQN\_persistent.ipynb}} (persistent-simulation primary; \texttt{lecture\_08\_09\_OLG\_Benchmark\_DEQN\_exogenous.ipynb} is the feedback-free ablation, same model and loss, different training cloud).  \S 1--4 match III.1--III.5; \S 5--7 = training and policy stability; \S 8 = plots/diagnostics of III.10--III.12.  Teaching runtime ($\sim$200 segments, 128-wide): a few minutes.
\end{frame}
