...
\subtitle{Finite-difference and PINN solvers for the HJB+KFE system}
...
\textbf{From the Lecture~12 theory deck}: We derived the \emphc{theory} of CT heterogeneous agent models:
...
\begin{itemize}
  \item Implement finite-difference schemes (upwind discretization) to solve HJB equations on wealth grids
  \item Compare finite-difference solutions with PINN and EMINN approaches, recognizing computational trade-offs (speed, accuracy, dimensionality)
  \item Deploy physics-informed neural networks to solve the coupled HJB+KFE system for stationary equilibrium in heterogeneous-agent economies
  \item Apply equation-constrained multi-output neural networks (EMINNs) to solve the master equation with aggregate shocks (Krusell--Smith)
  \item Validate deep-learning solutions against finite-difference benchmarks and assess the deep-learning conundrum in macro-finance
\end{itemize}
\end{frame}
...
\begin{itemize}\itemsep1pt
    \item $V_\theta$: value function network
    \begin{itemize}\itemsep0pt
        \item Input: $a$ (scalar)
        \item Output: 2-vector $\big(V(a,n_1),V(a,n_2)\big)$, one per income state
        \item Architecture: 4 hidden layers, 64 neurons, $\tanh$
    \end{itemize}
    \item $g_\phi$: density network
    \begin{itemize}\itemsep0pt
        \item Input: $a$ (scalar)
        \item Output: 2-vector $\big(g(a,n_1),g(a,n_2)\big) \geq 0$ (softplus)
...
\small
An alternative to recovering $c$ from derivatives of $V$ is to make the policy an explicit network output and let an FOC residual tie the two together.
\vspace{-0.3em}
\[
...
{\footnotesize It generalises to two discrete income states and to OU diffusion income; Notebook 08 instead takes the $c=(V_a)^{-1/\gamma}$ route with log utility ($\gamma=1$, so $c=1/V_a$) and outputs $(V,g)$ for the coupled HJB--KFE system.}
\end{frame}
...
\small
The interior HJB residual at collocation point $(a, n)$.  In notebook 08, consumption is recovered from the value derivative, $c^* = (V'_\theta)^{-1/\gamma}$.  In the explicit-policy variant of the previous slide, replace $c^*$ by the network output $c_\theta$ and add an FOC residual.
\vspace{-0.3em}
\[
...
\begin{block}{PINN Training at FD-Computed Equilibrium Prices (Notebook 08)}
\footnotesize
\setlength{\itemsep}{0pt}
\begin{algorithmic}[1]
\STATE Initialize $V_\theta$, $g_\phi$; fix prices at the FD equilibrium $(r^\star, w^\star)$
\FOR{$k = 1, \ldots, N_{\text{epochs}}$}
    ...
    \STATE Compute $\mathcal{L}_{\text{SC}}, \mathcal{L}_{\text{flux}}, \mathcal{L}_{\text{mass}}, \mathcal{L}_{\text{MC}}$ \;\;($\mathcal{L}_{\text{MC}}$ at fixed $r^\star$)
...
\textbf{Key choices:}
\begin{itemize}\itemsep0pt
    \item \textbf{Prices}: fixed at the FD $(r^\star, w^\star)$; outer $r\!\leftarrow\!r+\alpha(K^s\!-\!K^d)$ updates $\Rightarrow$ GE PINN
...
Same philosophy as the PINN lecture: \emphc{economic equations = loss function};
...
\vspace{0.3em}
\textbf{PINN diagnostics (notebook 08, longer in-class run):}\\[2pt]
{\scriptsize
...
\vspace{0.2em}
{\footnotesize \emphc{Honest read:} $K$ matches to 5 digits; pointwise density and KS gaps are non-trivial. The integrated-KFE term targets exactly the loosest residual. The checked-in \texttt{smoke} run is shorter and prints larger gaps.}
...
\begin{itemize}\itemsep0pt
    ...
    \item Recall Chapter~7 (PINNs), Notebook~02
\end{itemize}
\end{block}
\end{columns}
...
\vspace{0.05em}
{\scriptsize\emph{Schematic.}  Curves are illustrative; actual PINN outputs from NB~08 are slightly noisier near the borrowing limit and in the right tail.}
\end{frame}
...
\vspace{0.2em}
\begin{block}{The Master Equation (Lecture~12)}
\small
...
(e.g., Young's method from Chapter~6 used $\sim 10{,}000$ agents).\\
...
\begin{frame}[shrink=10]{Neural Network for $W := \partial_a V$}
\small
...
\vspace{0.05em}
\footnotesize
\textbf{Q\&A: Key differences from discrete time?}
\begin{itemize}\itemsep0pt
...
\vspace{0.05em}
{\scriptsize\emph{Schematic.}  Curves are illustrative; for the actual NB~08 outputs (with PINN jitter visible near $\ubar a$ and in the right tail) run the notebook end to end.}
\end{frame}
...
% --- Slide: KS Time Paths ---
\begin{frame}[shrink=20]{Krusell--Smith: Sample Time Paths}
\begin{columns}[T]
\column{0.48\textwidth}
\begin{center}
\begin{tikzpicture}
...
\vspace{0.05em}
{\scriptsize\emph{Schematic.}  Curves are illustrative impulse responses, not exports from a trained EMINN run; for the actual reported numerics see Gu, Lauri\`ere, Merkel \& Payne (2024), Fig.~5--7.}
\end{frame}
...
\node[box] (d2) at (0,3.5) {DEQNs (discrete)};
\node[box] (d4) at (0,2.6) {Young's (discrete)};
\node[box] (d5) at (0,1.7) {PINN single-PDE lecture};
\node[current] (d6a) at (0,0.8) {PINNs for HJB+KFE};
\node[current] (d6b) at (0,-0.1) {EMINNs (master eq.)};
...
\centering\small
\vspace{0.1em}
\emphc{Mesh-free} solution of HJB + KFE via auto-diff. Same philosophy as Chapter~7.
\vspace{0.1em}
\end{block}
...
\vspace{0.1em}
\begin{center}
\textbf{Next chapter}: Surrogate models \& Gaussian Processes (Chapter~9).
\end{center}
\end{frame}
...
\footnotesize
\textbf{Companion notebook for this lecture (self-contained, PyTorch):}
\begin{itemize}\itemsep2pt
    \item \textbf{NB 08} \textit{(in-class)}: \texttt{lecture\_13\_08\_Aiyagari\_Continuous\_Time\_FD\_and\_PINN\_PyTorch.ipynb} --- the stationary Aiyagari equilibrium: an upwind finite-difference solver computes the GE prices, then a PINN re-solves the coupled HJB+KFE system at those prices, with FD as the validation benchmark.
\end{itemize}

\vspace{0.2em}
\textbf{Suggested in-class flow:} run the notebook top to bottom --- (i) FD dense baseline and the sparse-FD cross-check, (ii) the GE bisection on $r$, (iii) the PINN re-solve at $(r^\star, w^\star)$, (iv) the PINN-vs-FD agreement diagnostics. The single-PDE HJB warm-ups (discrete-income and OU-diffusion) are PINN exercises and live with the PINN lecture (Lecture~11).
\end{frame}
